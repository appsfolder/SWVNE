  ...
  unlockAudio() {
    ...
    if (promise !== undefined) {
      promise
        ...
        .catch((error) => {
          console.warn(
            "Audio unlock failed, will try again on next user interaction.",
            error,
          );
        });
    }
...
class VisualNovelEngine {
  ...
  async initializeEngine() {
    ...
    if (scenarioToStart && this.gameData.scenarios[scenarioToStart]) {
      console.log(
        `Found scenario '${scenarioToStart}' from URL parameter. Preparing start screen.`,
      );
      this.prepareScenarioStartScreen(scenarioToStart);
    } else {
      this.showMainMenu();
    }
  ...
  async loadContentFromServer() {
    try {
      const [charactersResponse, scenesResponse, scenariosResponse] =
        await Promise.all([
  ...
  prepareScenarioStartScreen(scenarioId) {
    ...
    document.getElementById("scenarioStartTitle").textContent =
      scenario.title || "Безымянный сценарий";
    document.getElementById("scenarioStartDescription").textContent =
      scenario.description || "Нет описания.";

    const authorEl = document.getElementById("scenarioStartAuthor");
    if (scenario.author) {
      authorEl.textContent = `Автор: ${scenario.author}`;
      authorEl.style.display = "block";
    } else {
      authorEl.style.display = "none";
    }

    const startButton = document.getElementById("scenarioStartButton");
    const newStartButton = startButton.cloneNode(true);
    startButton.parentNode.replaceChild(newStartButton, startButton);

    newStartButton.addEventListener("click", () => this.startGame(scenarioId));

    this.showScreen("scenarioStartScreen");
  }
  ...
  showScenarioSelection() {
    ...
    if (scenarios.length === 0) {
      scenarioList.innerHTML =
        "<p>Сценарии не найдены. Загрузите их через админ-панель.</p>";
    } else {
      scenarios.forEach(([id, scenario]) => {
        ...
        const authorHtml = scenario.author
          ? `<span class="scenario-author">Автор: ${scenario.author}</span>`
          : "";
  ...
  async loadSavedGame() {
    ...
    try {
      ...
      if (
        savedState &&
        savedState.currentScenario &&
        savedState.currentDialogue
      ) {
  ...
  async displayDialogue(dialogueId) {
    ...
    if (dialogue.character && this.gameData.characters[dialogue.character]) {
      const char = this.gameData.characters[dialogue.character];
      this.setCharacterName(char.name, char.color);
    } else {
  ...
  setNextAction(callback) {
    this.nextActionCallback = callback;
    document.getElementById("clickIndicator").classList.remove("hidden");
    if (this.autoMode && callback) {
      setTimeout(() => {
  ...
  showChoices(choices, speakerId) {
    ...
    const availableChoices = choices.filter((choice) =>
      this.checkCondition(choice.condition),
    );
    ...
    if (availableChoices.length > 0) {
      ...
      if (
        this.gameData.scenarios[this.gameState.currentScenario].dialogues[
  ...
  updateHistory(dialogue) {
    this.gameState.history.push({
      id: this.gameState.currentDialogue,
      character: dialogue.character
        ? this.gameData.characters[dialogue.character]?.name
        : "Сюжет",
      text: dialogue.text,
  ...
  setCharacterName(name, color = "#20c997") {
  ...
  async typeText(text) {
    ...
    for (let i = 0; i < text.length; i++) {
      ...
      await new Promise((resolve) =>
        setTimeout(resolve, this.settings.textSpeed),
      );
    }
    this.isTyping = false;
    dialogueTextElement.innerHTML = text;
  }
  completeTyping() {
    this.isTyping = false;
  }
  ...
  saveSettings() {
    localStorage.setItem("vnSettings", JSON.stringify(this.settings));
  }
  updateUI() {
    const textSpeedSlider = document.getElementById("textSpeed");
    if (textSpeedSlider) textSpeedSlider.value = this.settings.textSpeed;
    const textSpeedValue = document.getElementById("textSpeedValue");
    if (textSpeedValue)
      textSpeedValue.textContent = `${this.settings.textSpeed}ms`;
  ...
  toggleAutoMode() {
    ...
    const autoButton = document.querySelector(
      '.ui-btn[onclick="toggleAutoMode()"]',
    );
    if (autoButton) {
      autoButton.textContent = this.autoMode ? "⚡ Авто (ВКЛ)" : "⚡ Авто";
    }
  }
  showHistory() {
    const historyContent = document.getElementById("historyContent");
    historyContent.innerHTML = "";
    this.gameState.history.slice(-20).forEach((item) => {
      const historyItem = document.createElement("div");
      historyItem.className = "history-item";
      historyItem.innerHTML = `<div class="history-character">${item.character} (${item.timestamp})</div><div>${item.text}</div>`;
  ...
  autoSave() {
    localStorage.setItem("vnAutoSave", JSON.stringify(this.gameState));
  }
  showGameScreen() {
    this.showScreen("gameScreen");
  }
...
function showMainMenu() {
  if (engine) engine.showMainMenu();
}
function showScenarioSelection() {
  if (engine) engine.showScenarioSelection();
}
function startGame(id) {
  if (engine) engine.startGame(id);
}
function loadGame() {
  if (engine) engine.loadSavedGame();
}
function showSettings() {
  if (engine) engine.showSettings();
}
function saveGame() {
  if (engine) engine.saveGame();
}
function toggleAutoMode() {
  if (engine) engine.toggleAutoMode();
}
function showHistory() {
  if (engine) engine.showHistory();
}
function hideHistory() {
  if (engine) engine.hideHistory();
}
function updateTextSpeed(value) {
  if (engine) {
    engine.settings.textSpeed = parseInt(value, 10);
    document.getElementById("textSpeedValue").textContent = `${value}ms`;
    engine.saveSettings();
  }
}
function toggleAutoPlay(checked) {
  if (engine) {
    engine.settings.autoPlay = checked;
    engine.saveSettings();
  }
}
function toggleFullscreen(checked) {
  if (engine) {
    engine.settings.fullscreen = checked;
    engine.saveSettings();
    if (checked && document.documentElement.requestFullscreen) {
      document.documentElement.requestFullscreen();
    } else if (!checked && document.exitFullscreen) {
