document.addEventListener("DOMContentLoaded", () => {
  ...
  // Preview modal
  const locationPreviewOverlay = document.getElementById(
    "location-preview-overlay",
  );
  const locationPreviewModal = document.getElementById(
    "location-preview-modal",
  ); // Можно оставить, если понадобится
  const locationPreviewImage = document.getElementById(
    "location-preview-image",
  );
  const locationPreviewTitle = document.getElementById(
  ...
  if (missingElements.length > 0) {
    console.error(
      "Asset Editor: Missing required DOM elements:",
      missingElements.map((item) => item.name),
    );
    if (typeof notifications !== "undefined") {
      notifications.error(
        "Ошибка инициализации",
        `Не найдены элементы: ${missingElements.map((item) => item.name).join(", ")}`,
      );
    }
  ...
  function renderAudioAssets(type, container) {
    ...
    assets.forEach((asset) => {
      ...
      card.querySelector(".asset-size").textContent = formatFileSize(
        asset.size,
      );
      ...
      deleteBtn.addEventListener("click", () =>
        deleteAsset(type, asset.path, asset.name),
      );
  ...
  function renderLocationAssets() {
    ...
    locations.forEach((location) => {
      const cardFragment = locationTemplate.content.cloneNode(true);
      const card = cardFragment.firstElementChild; // Работаем с элементом, а не с фрагментом

      // Используем location name если доступно, иначе filename
      const displayName =
        location.name || location.filename || "Unknown Location";
      ...
      // Убедимся, что кнопка найдена, прежде чем вешать обработчик
      if (previewBtn) {
        // Preview functionality
        previewBtn.addEventListener("click", () => {
          if (!locationPreviewOverlay) return;
          locationPreviewTitle.textContent = `Предпросмотр: ${displayName}`;
          locationPreviewImage.src = location.path;
          locationPreviewOverlay.classList.add("show");
        });
      } else {
        console.error(
          "Preview button not found in location template for asset:",
          location,
        );
      }

      // Edit functionality (только для локаций с ID)
      if (editBtn) {
        if (location.id) {
          editBtn.addEventListener("click", () =>
            editLocationName(location.id, displayName),
          );
        } else {
          editBtn.style.display = "none"; // Скрываем кнопку, если нет ID
        }
      }

      // Delete functionality
      if (deleteBtn) {
        const deleteDisplayName = location.id
          ? `${displayName} (${locationId})`
          : displayName;
        deleteBtn.addEventListener("click", () =>
          deleteAsset("locations", location.path, deleteDisplayName),
        );
      }

      locationsGrid.appendChild(cardFragment);
    });
  }
  ...
  async function editLocationName(locationId, currentName) {
    const newName = await notifications.prompt(
    ...
    try {
      ...
      if (result.success) {
        ...
        notifications.success(
          "Обновление завершено",
          `Название локации изменено на "${newName.trim()}"`,
        );
      } else if (result.requires_auth) {
        // Handle authentication required - prompt for login
        const authenticated = await auth.ensureAuthenticated();
  ...
  function exportAssets() {
    ...
    notifications.success(
      "Экспорт завершён",
      "Список ресурсов сохранен в файл",
    );
  }
  ...
  function importAssets(file) {
    ...
    reader.onload = async function (e) {
      try {
        ...
        if (!data.assets) {
          throw new Error(
            'Неверный формат файла. Отсутствует секция "assets".',
          );
        }
        ...
        message +=
          "Примечание: Это информационный импорт. Сами файлы нужно загружать отдельно.";
  ...
  function closeLocationPreview() {
    if (locationPreviewOverlay) {
      locationPreviewOverlay.classList.remove("show");
    }
  }

  if (closePreviewBtn) {
    closePreviewBtn.addEventListener("click", closeLocationPreview);
  }

  if (locationPreviewOverlay) {
    locationPreviewOverlay.addEventListener("click", (e) => {
      if (e.target === locationPreviewOverlay) {
